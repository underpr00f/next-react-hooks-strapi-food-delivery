...
import { cookiesParser } from "../lib/cookiesParser";
...
class MyApp extends App {
  static async getInitialProps({ Component, router, ctx }) {
    let pageProps = {};
    const cookies = cookiesParser(ctx.req);
    const isDark = cookies && cookies.isDark;
    console.log(isDark);
  ...
  setTheme = (isDark) => {
    ...
    Cookie.set("isDark", JSON.stringify(isDark));
    this.setState({ isDark: isDark });
  };
  addItem = (objectItem) => {
        ...
        >
          <Head>
            ...
            <meta
              name="theme-color"
              content={
                theme(JSON.parse(this.props.isDark || isDark)).palette.primary
                  .main
              }
            />
          </Head>
          <ThemeProvider theme={theme(JSON.parse(this.props.isDark || isDark))}>
